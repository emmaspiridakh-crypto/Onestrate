...
# ROLE IDs
OWNER_ID               = 1486666952570175538
CO_OWNER_ID            = 1486667057956388985
DEVELOPER_ID           = 1490765300981829855
MANAGER_ID             = 1486667805641146449
STAFF_ID               = 1486668549266346115
JOBORG_ID              = 1490765513629106187
DUTY_ROLE_ID           = 1490767647720345733

# AUTOROLE
AUTOROLE_ID = 1486670800684585012

# ============ WHITELIST / APPLICATIONS ============
STAFF_APP_ROLE_ID            = 1486668918314504293
MANAGER_APP_ROLE_ID          = 1486667805641146449

STAFF_RESULTS_CHANNEL_ID     = 1490761471615963177
STAFF_CATEGORY_ID            = 1490762503179862310

MANAGER_RESULTS_CHANNEL_ID   = 1490761498438402118
MANAGER_CATEGORY_ID          = 1490762566832754698

DUTY_PANEL_CHANNEL_ID        = 1490763798963945656
DUTY_LOG_CHANNEL_ID          = 1490761720975855851
DUTY_LEADERBOARD_CHANNEL_ID  = 1490761720975855851

SECURITY_LOG_CHANNEL_ID      = 1490761449797324862
...
# ============ PANEL IMAGES ============
SERVER_BANNER_URL    = "https://i.imgur.com/C3KQv3j.jpeg"
SERVER_THUMBNAIL_URL = "https://i.imgur.com/C3KQv3j.jpeg"

# ROLES ΠΟΥ ΜΠΟΡΟΥΝ ΝΑ ΚΑΝΟΥΝ ACCEPT/DENY
APPLICATION_MANAGER_ROLES = [ OWNER_ID, CO_OWNER_ID]
...
# CATEGORY IDs
MAIN_TICKET_CATEGORY_ID = 1486672756555780236
JOB_TICKET_CATEGORY_ID  = 1486673255657111572

# TEMP VOICE
TEMP_VOICE_CATEGORY_ID = 1490762777390878820
TEMP_VOICE_CHANNEL_ID  = 1486673722764169286

# LOG CHANNELS
ON_OFF_DUTY_LOG_ID            = 1490761720975855851
BOT_LOG_ID                    = 1490761221861806340
MESSAGE_EDIT_LOG_CHANNEL_ID   = 1490761163745525790
MESSAGE_DELETE_LOG_CHANNEL_ID = 1490761163745525790
MEMBER_JOIN_LOG_CHANNEL_ID    = 1490761205915062445
MEMBER_LEAVE_LOG_CHANNEL_ID   = 1490761205915062445
ROLE_UPDATE_LOG_CHANNEL_ID    = 1490761620702630028
VOICE_LOG_CHANNEL_ID          = 1490761185551716462
CHANNEL_CREATE_LOG_CHANNEL_ID = 1490761528134074548
CHANNEL_DELETE_LOG_CHANNEL_ID = 1490761528134074548
ROLE_CREATE_LOG_CHANNEL_ID    = 1490761620702630028
ROLE_DELETE_LOG_CHANNEL_ID    = 1490761620702630028
TICKET_LOG_ID                 = 1490769292348887090

# VOICE COUNTER CHANNELS
MEMBERS_CHANNEL_ID = 1490769435420786890
BOTS_CHANNEL_ID    = 1490769493000061222
ONLINE_CHANNEL_ID  = 1490769465804066926
BOOSTS_CHANNEL_ID  = 1490769520405516339
...
class MainTicketSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.values[0] == "Owner":
            ...
            roles_ids   = [MANAGER_ID, OWNER_ID, CO_OWNER_ID]
...
async def _track_mass_action(guild: discord.Guild, moderator, action_type: str):
    ...
    if len(ban_kick_tracker[uid]) >= 3:
        ...
        exempt_ids = [CO_OWNER_ID, OWNER_ID]
...
async def on_message(message: discord.Message):
    ...
    if guild and URL_PATTERN.search(message.content):
        exempt_roles = [CO_OWNER_ID, OWNER_ID]
...
def has_staff_permissions(member: discord.Member):
    return (
        ...
        any(r.id in (STAFF_ID, MANAGER_ID, OWNER_ID, CO_OWNER_ID,) for r in member.roles)
    )
...
async def dmall(ctx, *, message: str):
    founder_role = ctx.guild.get_role(OWNER_ID)
...
async def panel(ctx):
    if not is_owner_or_coowner(ctx.author):
        return await ctx.reply("❌ Δεν έχεις δικαίωμα.")
    embed = discord.Embed(title="📌 Onestrate Remastered — Command Panel",
    ...
    embed.add_field(name="📋 Applications", value="`!applicationpanel` (Staff/Manager)",   inline=False)
...
async def ticketpanel(ctx):
    if not is_owner_or_coowner(ctx.author):
        return await ctx.reply("Δεν έχεις δικαίωμα.")
    embed = discord.Embed(
        title="Onestrate Remastered — Support Panel",
        description=(
    ...
    embed.set_image(url=SERVER_BANNER_URL)
    embed.set_thumbnail(url=SERVER_THUMBNAIL_URL)
    embed.set_footer(text="Onestrate Remastered • Support System")
...
async def jobpanel(ctx):
    if not is_owner_or_coowner(ctx.author):
        return await ctx.reply("Δεν έχεις δικαίωμα.")
    embed = discord.Embed(
        title="Onestrate Remastered — Job Panel",
        description=(
    ...
    embed.set_image(url=SERVER_BANNER_URL)
    embed.set_thumbnail(url=SERVER_THUMBNAIL_URL)
    embed.set_footer(text="Onestrate Remastered • Job System")
...
async def applicationpanel(ctx):
    ...
    if not is_owner_or_coowner(ctx.author):
        return await ctx.reply("Δεν έχεις δικαίωμα.")
    embed = discord.Embed(
        title="📋 Onestrate Remastered — Applications",
        description=(
    ...
    embed.set_image(url=SERVER_BANNER_URL)
    embed.set_thumbnail(url=SERVER_THUMBNAIL_URL)
    embed.set_footer(text="Onestrate Remastered • Applications System")
...
async def suggestionpanel(ctx):
    ...
    embed.set_image(url=SERVER_BANNER_URL)
    embed.set_thumbnail(url=SERVER_THUMBNAIL_URL)
    embed.set_footer(text="Onestrate Remastered • Suggestion System")
...
async def reviewpanel(ctx):
    ...
    embed.set_image(url=SERVER_BANNER_URL)
    embed.set_thumbnail(url=SERVER_THUMBNAIL_URL)
    embed.set_footer(text="Onestrate Remastered • Review System")
...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Game(name="Onestrate Remastered"))
